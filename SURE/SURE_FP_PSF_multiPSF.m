function out = SURE_FP_PSF_multiPSF(Ihat,fhat,opts)
...
for i = 1:iter
    ...
    numer = sigma^2*lambda*V.*hhat.*(M.^2);
    ...
    traceLambda = sum(hhat2(:).*V(:)./denom(:).^2);
    ...
    lambda = real((m*sigma^2*traceLambda)/sum(col(conj(denom1).*denom2)));
