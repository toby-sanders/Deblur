...
b = b(1:K:end,1:K:end);
bF = zeros(size(b,1),size(b,2),Nf);
for i = 1:Nf
    bF(:,:,i) = add_Wnoise(b,SNR);
end

[b,sigma] = add_Wnoise(b,SNR);
bstr = zeros(d0,d0);
bstr(1:K:end,1:K:end) = b;
bFstr = zeros(d0,d0,Nf);
bFstr(1:K:end,1:K:end,:) = bF;
...
hhat2 = abs(hhat).^2;
ghat2 = abs(ghat).^2;
filtF = conj(hhat).*conj(ghat)./(Nf*hhat2.*ghat2/K^2 + lambda*V*Nf);
u5 = real(ifft2(fft2(sum(bFstr,3)).*filtF));
sigmaPSF = sigma^2*filtF;
u6 = GBM3D(u5,sigmaPSD,BMopts);
...
t3 = nexttile;
imagesc(u4,[0 1]);
title(sprintf('Wiener rec: PSNR = %g',myPSNR(x,u3,1)))
t4 = nexttile;
imagesc(u6,[0 1]);
title(sprintf('P3 rec: PSNR = %g',myPSNR(x,u6,1)));
