...
for i = 1:iter
    ...
    if i>3
        ...
        Snum = omegaX - omegaXp;
        Yden = real(numerX + denomX) - gX;
        % tauX = Snum/Yden;

        Snum = omegaY - omegaYp;
        Yden = real(numerY + denomY) - gY;
        % tauY = Snum/Yden;
    ...
    gX = real(numerX + denomX);
    gY = real(numerY + denomY);
    gT = real(numerT + denomT);

    omegaX = max(omegaX - tauX*gX,.1);
    omegaY = max(omegaY - tauY*gY,.1);
    theta =  theta - tau*gT;% 
    ...
    rel_chg = abs(lambda - out.lambdas(end-1))/lambda;
    out.rel_chg = [out.rel_chg;rel_chg];
    if rel_chg<opts.tol, break; end



    gam = min(gam+(.05),gamMax);
end
