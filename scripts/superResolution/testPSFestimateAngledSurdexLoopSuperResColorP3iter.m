...
for i = 1:M
    for j = 1:N
        ...
        A = getSuperResDeblurOpers(m*K,n*K,K,fhat,ghat);
        Reg = getGBM3D;
        popts.sigma0 = sigma*0.85;% 1/255;
        popts.iter = 20;
        popts.tol = 1e-5;
        
        p3rec = zeros(m*K,n*K,3);
        for cc = 1:3
            popts.init = tmp3(:,:,cc);
            [p3rec(:,:,cc),p3out] = PnP3_prox(A,Reg,col(bC(:,:,cc)),[m*K,n*K,1],popts);
        end
        ...
        p3rec = p3rec(1:end-pad*K,1:end-pad*K,:);
        ...
        recP3(indYS,indXS,:) = recP3(indYS,indXS,:) + p3rec.*wS;
