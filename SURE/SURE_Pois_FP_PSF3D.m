function out = SURE_Pois_FP_PSF3D(Ihat,omega,opts)
...
for i = 1:iter
    ...
    numer = (Ihat(1)/m)*lambda*V.*hhat./(hhat2 + lambda*V).^2;
    numerX = sum(Dx(:).*numer(:));
    numerY = sum(Dy(:).*numer(:));
    numerT = sum(Dt(:).*numer(:));
    ...
    traceLambda = Ihat(1)/m*sum(hhat2(:).*V(:)./(hhat2(:)+V(:)*lambda).^2);

    % new lambda updated based on the fixed point
    denom = V.*rec; denom2 = denom./(hhat2+V*lambda);
    lambda = real((m*traceLambda)/sum(col(conj(denom).*denom2)));
