function [psf,Fpsf,Dh] = makeGausPSF(n,sigma,pixelSize)
...
% derivative of Fpsf, with respect to sigma
Dh = (exp(-2*sigma^2*(pi*X/d2).^2 - 2*sigma^2*(pi*Y/d1).^2));
Dh = ifftshift(Dh.*(-4*sigma*(pi*X/d2).^2 - 4*sigma*(pi*Y/d1).^2));